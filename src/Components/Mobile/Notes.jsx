    ...
    <>
      <div className="notesmb">
        <h2 style={{ backgroundColor: '#001F8B' }}>
          <div onClick={back} className="back"><img src={backbtn} alt="back" /></div>
          <button className='grpbtnmb' style={{ backgroundColor: selected.bgcolor, color: 'white', fontWeight: '500', fontSize: '1em', padding: '0', width: '2.6em', height: '2.6em' }}>{selected.display}</button>
          {selected.name}
        </h2>
                ...
                return (
                  <div key={i} className="localnotemb">
                    ...
                    <div className='timestampmb'>
                      <p>{note.date}</p>
                      <p style={{ backgroundColor: 'black', width: '6px', height: '6px', borderRadius: '50%' }}></p>
                      <p>{note.time}</p>
                    </div>
                  </div>
                );
        ...
        <div className='noteinputmb' style={{ backgroundColor: '#001F8B' }}>
          ...
          <button className='sendmsgmb' onClick={sendnote}><img src={msgsend} style={{ opacity: send? 1 : 0.4, cursor: send? 'pointer' : 'not-allowed' }} alt="send" /></button>
        </div>
      </div>
    </>
  );
