...
function Home() {
  ...
  return (
    <>
      <div className="home">
        <h1 style={{ position: 'sticky', top: '0', backgroundColor: 'white', width: '90vw', paddingLeft: '20vw' }}>Pocket Notes</h1>
        {group.map((grp, index) => {
          return (
            <Link key={index} style={{ textDecoration: 'none' }} to='./notes'>
              <div className='list' key={index} style={{ backgroundColor: selected && selected.name === grp.name ? '#D4DEEE' : 'transparent', fontWeight: '500',transition: 'background-color 0.3s'  }} onClick={() => openM(grp)}>
                <button className='listicon' style={{ backgroundColor: grp.bgcolor,border:'none', color: 'white', fontWeight: '500', padding: '0' }}>{grp.display}</button>{grp.name}
              </div>
            </Link>
          );
        })}
        <div style={{ padding: '5vh', zIndex: '10', position: 'absolute', bottom: '0', right: '0' }}>
          <div onClick={e => setPop(true)} className='grpadd'>+</div>
        </div>
        {/* popup div here */}
        {pop ?
          (<div className="Popupmb">
            ...
            <div className='colorgrp'>
              <h2 style={{ marginBottom: '0', marginRight: '1em' }}>Group Name</h2>
            ...
            <div style={{ display: 'flex' }}>
              <h2 style={{ marginBottom: '2vh', marginRight: '0.3em' }}>Choose colour</h2>
              <div style={{ display: 'inline-block' }}>
                {['#6691FF', '#FF79F2', '#43E6FC', '#F19576', '#0047FF', '#B38BFA'].map((clr, index) => (
                  <span key={index} onClick={() => handleColor(clr, index)} style={{ justifyContent: 'space-between' }}>
                    <button className='color-option-mb' style={{ backgroundColor: clr, border: active === index ? '1.px solid black' : '1px solid white' }}>
                    </button>
                  </span>
                ))}
              </div>
